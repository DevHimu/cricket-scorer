...
// 1. LOGIN
// ---------------------------------------------------------------------------
const LS_TOKEN = 'cs_token', LS_MATCH = 'cs_matchId';
function persistSession() {
  if (state.token) localStorage.setItem(LS_TOKEN, state.token); else localStorage.removeItem(LS_TOKEN);
  if (state.matchId) localStorage.setItem(LS_MATCH, state.matchId); else localStorage.removeItem(LS_MATCH);
  updateAuthUI();
}
function clearMatchSession() { state.matchId = null; localStorage.removeItem(LS_MATCH); }
function logout() { state.token = null; state.matchId = null; localStorage.removeItem(LS_TOKEN); localStorage.removeItem(LS_MATCH); location.reload(); }
function updateAuthUI() { const b = $('logoutBtn'); if (b) b.style.display = state.token ? 'inline-block' : 'none'; }
...
// Restore a saved session on page load so a refresh doesn't force re-login,
// and resume an in-progress match if one was open.
async function restoreSession() {
  ...
  if (mid) {
    try {
      const m = await api('/api/matches/' + mid);
      if (m && m.status && m.status !== 'complete') {
        state.matchId = m.id; state.match = m;
        return resumeMatch(m);
      }
    } catch (_) {}
    clearMatchSession();
  }
...
$('startGameBtn').onclick = async () => {
  setErr('ovErr', '');
  try {
    const m = await api('/api/matches/create', 'POST', state.config);
    state.matchId = m.id; state.match = m;
    persistSession();
...
function showResult() {
  ...
  clearMatchSession(); // match finished — don't auto-resume it next load
